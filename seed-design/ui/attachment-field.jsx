 ...
 */
export const AttachmentInputPreset = React.forwardRef(({ samples, triggerClassName, countClassName }, ref) => {
  return (
    <SeedAttachmentInput.Container ref={ref}>
      <PresetTrigger samples={samples} className={triggerClassName} countClassName={countClassName} />
      <SeedAttachmentInput.ItemGroup>
        <SeedAttachmentInput.Context>
          {({ acceptedFileEntries }) =>
            acceptedFileEntries.map((fileEntry) => (
...
function PresetTrigger({ samples, className, countClassName }) {
